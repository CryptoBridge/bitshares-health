...
module.exports = class HealthCheck {
    ...
    async bitshares() {
        ...
        if ((now - time) > 60000) {
            throw new Error(`Blockchain out of sync at block ${response.head_block_number}, latest tx on ${new Date(time).toISOString().substr(0, 19).replace(/T/g, ' ')} (${Math.round((now - time) / 1000)} seconds behind).`);
        }
