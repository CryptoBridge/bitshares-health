...
module.exports = class HealthCheck {
    ...
    static reconnect(status) {
        if (status === 'closed') {
            console.info('Status connection: closed');
            const reconnectTimer = setInterval(() => {
                Apis.instance(this.ws, true).init_promise
                    .catch(err => console.log(`Error connecting to node: ${err.message}`));
    ...
    static async getInstance(options = {}, ws, es) {
        ...
        let api;
        try {
            Apis.setRpcConnectionStatusCallback(HealthCheck.reconnect.bind({ ws }));
